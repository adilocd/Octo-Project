...
def Class_distribution():
    ...
    #         height2 ,ha="center")
    labels = ['clean transaction','fraudulent transaction']
    plt.pie(df['Class'].value_counts().values,explode=[0.2,0.7],autopct='%.2f%%',labels=labels,colors=["Grey","red"],radius=1.3, textprops={'fontsize': 18})
    return fig
##############################################################################
...
def correlation_heatmap():
    ...
    s = sns.heatmap(df.corr(),cmap=plt.cm.Reds,annot=True,fmt='.1f',linewidths=.9)
